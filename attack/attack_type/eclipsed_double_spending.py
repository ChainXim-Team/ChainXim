...
class EclipsedDoubleSpending(aa.AttackType):
    ...
    def renew_stage(self, round):
        ## 1. renew stage
        bh = self.behavior
        newest_block, mine_input, imcoming_block_from_eclipse = bh.renew(miner_list = self.adver_list,
        ...
        if eclipse_update:
            ...
            # while(from_block != None 
            #       and from_block.blockhash != self._fork_block.blockhash
            #       and from_block.blockhead.miner in self.eclipsed_list_ids):
            #     if not block.isAdversaryBlock and block.blockhead.miner not in self.eclipsed_list_ids:
            #         break
    ...
    def attack_stage(self, round, mine_input):
        ...
        if eclipse_height > adver_height:
            ...
            if self._eclipse_block_from.isAdversaryBlock:
                ...
                # 这里用add block 替代 
                '''先保证正确性 后续再用adopt替换'''
                self.adver_chain._add_block_forcibly(block=self._eclipse_block)
        ...
        self._log['eclipse_miner_block'] = self.eclipsed_list[0].consensus.local_chain.get_last_block().name
    ...
    def clear_record_stage(self, round):
        ...
        # self._log['eclipse_block'] = (self._eclipse_block.name ,self._eclipse_block.height) if self._eclipse_block != None else (None,0)
        # self._log['eclipse_miner_block'] = self.eclipsed_list[0].consensus.local_chain.get_last_block().name
        # self._log['eclipse_block'] = self._eclipse_block.name if self._eclipse_block != None else None
        # self._log['eclipse_from'] = self._eclipse_block_from.name if self._eclipse_block_from != None else None
    ...
    def __success_rate(self):
        if self._log['success'] != 0 or self._log['fail'] != 0:
            ...
            beta = (len(self.adver_list)+len(self.eclipsed_list))/(len(self.miner_list)-len(self.adver_list)-len(self.eclipsed_list))
            if ng > 100*n:
                # Ng 非常大
                if beta >= 1:
                    thr_rate = 1
