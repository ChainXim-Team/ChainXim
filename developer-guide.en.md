...
- **double_spending_success_times**, the number of successful double-spending attacks. A successful attack is defined as follows: during a fork event triggered by attackers, if the blockchain ultimately selects a branch led by the attackers as the main chain, and the first block of that adopted branch was generated by an attacker node, the attack is considered successful. This metric counts the total number of such successful attacks throughout the simulation.
